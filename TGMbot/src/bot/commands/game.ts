...
export function registerGameCommands(bot: Telegraf) {
  // Comando START - Benvenuto
  bot.command('start', (ctx) => {
    const welcomeMsg =
      '🎭 *Benvenuto a UNDERCOVER!*\n\n' +
      '*Come si gioca:*\n' +
  ...
  bot.command('newgame', async (ctx) => {
    ...
    await ctx.reply(
      '🎮 *Nuova partita Undercover creata!*\n\n' +
      '📝 *Istruzioni:*\n' +
  ...
  bot.command('join', async (ctx) => {
    ...
    if (!targetSession) {
      await ctx.reply(
        '⚠️ Non ci sono partite in attesa di giocatori.\n\n' +
  ...
  bot.command('startgame', async (ctx) => {
    ...
    if (started) {
      ...
      gameInfo += `\nOgni giocatore ha ricevuto ruolo e parola in privato.\n\n` +
        ...
        `Quando siete pronti, usate /endround per votare.`;

      // Suggerisci randomicamente chi inizia a parlare
      const starter = gameManager.getRandomAlivePlayer(groupChatId);
      if (starter) {
        gameInfo += `\n\n🎲 *Il bot suggerisce che ${starter.name} inizi la discussione!*`;
      }
  ...
  bot.command('endround', async (ctx) => {
    ...
    // Qualsiasi giocatore può terminare il round
    ...
    const reminderCallback = async () => {
      ...
      const notVotedPlayers = Array.from(session.players.values())
        .filter((p: Player) => p.isAlive && !p.hasVoted);

      if (notVotedPlayers.length > 0) {
        const names = notVotedPlayers.map((p: Player) => p.name).join(', ');
        await bot.telegram.sendMessage(
          groupChatId,
          `⏰ *Reminder votazione!*\n\nMancano ancora ${notVotedPlayers.length} voti.\nGiocatori che devono votare: ${names}`,
          { parse_mode: 'Markdown' }
        );
      }
    };
    ...
    if (votingStarted) {
      await ctx.reply(
        ...
        `⏰ Riceverete un reminder ogni 3 minuti se non tutti hanno votato.`,
        { parse_mode: 'Markdown' }
      );
      ...
      for (const voter of alivePlayers) {
        const buttons = alivePlayers
          .filter((p: Player) => p.id !== voter.id) // Non puoi votare te stesso
          .map((p: Player) => {
        ...
        try {
          await bot.telegram.sendMessage(
            voter.id,
            `🗳️ *Votazione Round ${session.currentRound}*\n\nChi vuoi eliminare?`,
            {
              parse_mode: 'Markdown',
  ...
  bot.command('status', async (ctx) => {
    ...
    const alivePlayers = allPlayers.filter((p: Player) => p.isAlive);
    const deadPlayers = allPlayers.filter((p: Player) => !p.isAlive);
          ...
          if (result) {
            ...
            await bot.telegram.sendMessage(
              ...
              `🗳️ *Risultato votazione:*\n\n` +
                ...
                `Parola: ${eliminatedPlayer?.word || 'Nessuna (Mr. White)'}`,
              { parse_mode: 'Markdown' }
            );
            ...
            if (winCondition.hasWinner) {
              ...
              const starter = gameManager.getRandomAlivePlayer(groupChatId);
              let nextRoundMsg = `\n🔄 *Round ${session.currentRound}*\n\nDiscutete! Poi usate /endround per votare.`;
              if (starter) {
                nextRoundMsg += `\n\n🎲 *Il bot suggerisce che ${starter.name} inizi la discussione!*`;
              }
              await bot.telegram.sendMessage(
